...
class EmailVerificationAPI {
   ...
   * - Path format: /verify/{id}/{hash}
   * - URL params: ?userId=...&token=... (current backend format)
   * - Legacy params: ?user_id=...&hash=...
  ...
  static parseVerificationURL(url = window.location.href) {
    ...
    // FIXED: Handle current backend format: ?userId=...&token=...
    const userId = params.get('userId') || params.get('user_id') || params.get('id');
    const hash = params.get('token') || params.get('hash');  // Now checks 'token' first!
    ...
    // Legacy token support (if needed for old URLs)
    const token = params.get('token');
    if (token && !userId) {  // Only use legacy if no userId found
      return {
        token,
        format: 'token'
      };
